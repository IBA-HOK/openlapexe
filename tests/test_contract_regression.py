# -*- coding: utf-8 -*-
"""Regression guard: old 95.8059 outside ±0.5% of 100.617, old R44 south diff >5s vs new 47.63.
...
- CANONICAL 100.61726451283548 s = f1/spa racing 50Hz measured via simulate_full("f1","spa",50);
  baseline txt data/reference/spa_f1_full_baseline.txt contains same value.
  TOL 0.5% => LO 100.1141781902713 HI 101.12035083544966.
...
- FULL_BASELINE_PATH data/reference/spa_f1_full_baseline.txt must contain 100.617...
  and be inside band, differ >1s from buggy.
- OLD_BASELINE_PATH data/reference/spa_f1_baseline.txt re-baselined to same canonical value
  2026-09-14 (old shim 101.17 in git history); file must be inside band for history.
...
def test_old_shim_baseline_still_101() -> None:
    ...
    # shim re-baselined 2026-09-14 must still be inside canonical band (history not broken)
    assert LO <= old <= HI, f"old shim {old} not in canonical band [{LO},{HI}]"
    # allow small diff vs canonical but still inside band
...
def test_old_buggy_95_outside_and_new_determinism() -> None:
    """Extra determinism guard: old 95.80 vs new 100.617 distinction must survive two runs."""
    from openlapexe.solver import simulate_full
    ...
    assert abs(float(r1.laptime) - PREVIOUS_BUGGY) > 4.0, "new vs buggy diff must be >4s (actual ~4.81s)"
